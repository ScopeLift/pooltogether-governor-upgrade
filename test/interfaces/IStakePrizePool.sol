...
interface IStakePrizePool {
  ...
  event Initialized(
    address reserveRegistry, uint256 maxExitFeeMantissa, uint256 maxTimelockDuration
  );
  ...
  event TimelockDeposited(
    address indexed operator, address indexed to, address indexed token, uint256 amount
  );
  ...
  event TimelockedWithdrawalSwept(
    address indexed operator, address indexed from, uint256 amount, uint256 redeemed
  );
  ...
  function awardExternalERC721(address to, address externalToken, uint256[] memory tokenIds)
    external;
  ...
  function compLikeDelegate(address compLike, address to) external;
